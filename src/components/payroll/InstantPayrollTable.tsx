...
import { Download, ExternalLink, Copy, Zap, CheckCircle, Clock, RefreshCw, AlertCircle, MoreVertical, Search, Filter, ArrowUpDown, Calendar, DollarSign, Activity, ArrowDown, ArrowUp, X, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
import { TransactionRecord } from '@/src/hooks/use-records';
...
interface InstantPayrollTableProps {
    records: TransactionRecord[];
    isSenderMode: boolean;
    onDownloadInvoice: (record: TransactionRecord) => void;
    onCopy: (text: string) => void;
    onViewTx?: (txHash: string) => void;
}

const InstantPayrollTable: React.FC<InstantPayrollTableProps> = ({
    records,
    isSenderMode,
    onDownloadInvoice,
    onCopy,
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'completed':
                return (
                    <Badge className="bg-green-100 text-green-800 hover:bg-green-200 border-green-200 flex items-center justify-center gap-1 text-xs px-2 py-1">
            ...
            case 'pending':
                return (
                    <Badge className="bg-yellow-100 text-yellow-800 hover:bg-yellow-200 border-yellow-200 flex items-center justify-center gap-1 text-xs px-2 py-1">
            ...
            case 'processing':
                return (
                    <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-200 border-blue-200 flex items-center justify-center gap-1 text-xs px-2 py-1">
            ...
            case 'failed':
                return (
                    <Badge className="bg-red-100 text-red-800 hover:bg-red-200 border-red-200 flex items-center justify-center gap-1 text-xs px-2 py-1">
    ...
    const filteredRecords = records
        .filter(record => {
            // Status filter
            if (statusFilter !== 'all') {
                const recordStatus = record.invoice?.invoiceStatus || 'completed';
        ...
        .sort((a, b) => {
            if (sortBy === 'date') {
                ...
                const aStatus = a.invoice?.invoiceStatus || 'completed';
                const bStatus = b.invoice?.invoiceStatus || 'completed';
                return sortDir === 'desc'
                    ? bStatus.localeCompare(aStatus)
                    : aStatus.localeCompare(bStatus);
            } else if (sortBy === 'name') {
                return sortDir === 'desc'
                    ? b.payrollName.localeCompare(a.payrollName)
                    : a.payrollName.localeCompare(b.payrollName);
            }
            return 0;
        });
    ...
    return (
        <>
                                ...
                                <div className="min-w-[140px] sm:min-w-[160px]">
                                    ...
                                    <Select value={statusFilter} onValueChange={v => setStatusFilter(v as any)}>
                                        ...
                                        <SelectContent className="bg-white/95 backdrop-blur-xl border border-white/60 shadow-2xl rounded-2xl">
                                            ...
                                            <SelectItem value="completed" className="hover:bg-green-50 rounded-xl">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                                                    Completed
                                                </div>
                                            </SelectItem>
                                            <SelectItem value="pending" className="hover:bg-yellow-50 rounded-xl">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                                                    Pending
                                                </div>
                                            </SelectItem>
                                            <SelectItem value="processing" className="hover:bg-blue-50 rounded-xl">
                                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="min-w-[120px] sm:min-w-[140px]">
                                        ...
                                        <Select value={sortBy} onValueChange={v => setSortBy(v as any)}>
                                            ...
                                            <SelectContent className="bg-white/95 backdrop-blur-xl border border-white/60 shadow-2xl rounded-2xl">
                                                ...
                                                <SelectItem value="name" className="hover:bg-indigo-50 rounded-xl">
                                                    <div className="flex items-center gap-2">
            ...
            <div className="rounded-xl overflow-hidden" style={{ height: '60vh', minHeight: '400px' }}>
                    ...
                    <Table className="h-full w-full">
                        <TableHeader>
                            <TableRow className="bg-gradient-to-r from-slate-50 via-blue-50/50 to-indigo-50/50 backdrop-blur-sm border-b border-slate-200 rounded-t-xl sticky top-0 z-10">
                                ...
                                <TableHead className="font-sans font-semibold text-gray-700 uppercase text-xs tracking-wide px-2 sm:px-4">Currency</TableHead>
                                <TableHead className="font-sans font-semibold text-gray-700 uppercase text-xs tracking-wide px-2 sm:px-4">Date</TableHead>
                                    ...
                                    >
                                                            ...
                                                            <Tooltip>
                                                                <TooltipTrigger asChild>
                                                                    <Button
                                                                        ...
                                                                        onClick={() => onCopy(isSenderMode ? record.receiverWalletAddress : record.senderWalletAddress)}
                                                                    >
                                        ...
                                        <TableCell className="px-2 sm:px-4">
                                            <div className="flex flex-row items-center gap-x-1">
                                                <span className="text-xs sm:text-sm text-slate-900 font-sans font-medium">
                                                    {formatDateTime(record.createdAt).date}
                                                </span>
                                            </div>
                                        </TableCell>
                                        <TableCell className="px-2 sm:px-4">
                                            {getStatusBadge(record.invoice?.invoiceStatus || 'completed')}
                                        </TableCell>
                                        <TableCell className="px-2 sm:px-4">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    ...
                                                    {record.txHash && onViewTx && (
                                                        <DropdownMenuItem
                                                            onClick={e => {
                                                                e.stopPropagation();
                                                                onViewTx(record.txHash);
                                                            }}
                                                            className="flex items-center gap-2"
                                                        >
...
export default InstantPayrollTable; 